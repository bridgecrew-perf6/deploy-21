...
contract OkxLink is Ownable, Pausable {
    using SafeMath for uint256;
    using SafeMath for uint256;
    ...
    IKIP17 private district;
    uint256 startBlockNumber;

    Account[] public accountArray;
    mapping(address => Account) public accounts;
    mapping(address => bool) public registered;

    constructor(IKIP17 _district) public {
        district = _district;
    }

    function regist(string memory uid) public whenNotPaused {
        //  require(district.balanceOf(msg.sender) > 0, "Need district");
        accounts[msg.sender] = Account(msg.sender, uid);
        if (registered[msg.sender] == false) {
            registered[msg.sender] = true;
            accountArray.push(Account(msg.sender, uid));
        }
    }

    function getByAddress(address wallet)
        public
        view
        returns (string memory uid)
    {
        if (registered[wallet] == false) {
            return "";
        }
        return accounts[wallet].uid;
    }

    function getAccounts() public view returns (Account[] memory ret) {
        return accountArray;
    }
    ...
    {
        return (accountArray[index].wallet, accountArray[index].uid);
    }
}