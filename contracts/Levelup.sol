...
contract Levelup is Ownable, Pausable, AdminRole {
    ...
    mapping(uint256 => LevelUpCost) levelUpCosts;

    event LevelUp(address indexed account, uint256 indexed tokenId, uint256 oldLevel, uint256 newLevel, uint256 layCost, uint256 orbCost);
    event Recovered(address tokenAddress, uint256 tokenAmount);

    constructor(IDistrictInfo _districtInfo, IKIP17Enumerable _district, IDistrictStaking _districtStaking, IKIP7 _lay, IKIP7 _orb, LevelUpCost [] memory costs) public {
    ...
    function setContract(IDistrictInfo _districtInfo, IKIP17Enumerable _district, IKIP7 _lay, IKIP7 _orb) public onlyOwner {
        districtInfo = _districtInfo;
        district = _district;
        lay = _lay;
        orb = _orb;
    ...
    function setVariable(LevelUpCost [] memory costs) public onlyAdmin {
        maxLevel = 0;
        for (uint256 i = 1; i <= maxLevel; i++) {
            delete levelUpCosts[i];
        }
    ...
    function renounceAdmin() public onlyAdmin {
        districtInfo.renounceAdmin();
    }

    function levelUp(uint256 tokenId) public payable whenNotPaused {
        address tokenOwner = district.ownerOf(tokenId);
        uint256 [] memory tokenIds = new uint256[](1);
        tokenIds[0] = tokenId;
        IDistrictStaking.StakingInfo [] memory stakingInfos = districtStaking.getStakingInfos(tokenIds);
        address stakedTokenOwner = address(0);
        if (stakingInfos.length > 0) {
            stakedTokenOwner = stakingInfos[0].owner;
        }

        require(tokenOwner == msg.sender || stakedTokenOwner == msg.sender, "the token is not yours");
        
        uint256 oldLevel = districtInfo.getAttribute(tokenId, "Level").toInt();

        require(oldLevel <= maxLevel, "the token is max level");

        LevelUpCost memory cost = levelUpCosts[oldLevel];

        require(cost.lay > 0 || cost.orb > 0, "no levelup cost");
        
        lay.transferFrom(msg.sender, address(this), cost.lay);
        orb.transferFrom(msg.sender, address(this), cost.orb);
        
        uint256 newLevel = oldLevel + 1;
        districtInfo.setAttribute(tokenId, "Level", newLevel.toString());

        emit LevelUp(msg.sender, tokenId, oldLevel, newLevel, cost.lay, cost.orb);
    }

    function getCosts() public view returns (LevelUpCost [] memory) {
        LevelUpCost [] memory result = new LevelUpCost[](maxLevel);
        for (uint256 i = 1; i <= maxLevel; i++) {
            result[i-1] = levelUpCosts[i];
        }
        return result;
    }

    function recoverKIP7(address tokenAddress, uint256 tokenAmount) external payable onlyAdmin {
        IKIP7(tokenAddress).transfer(msg.sender, tokenAmount);
        emit Recovered(tokenAddress, tokenAmount);
    }

    function setCosts(LevelUpCost [] memory costs) private {
        for (uint256 i = 0; i < costs.length; i++) {
            uint256 level = i + 1;
            levelUpCosts[level] = costs[i];
            if (level > maxLevel) {
                maxLevel = level;
            }
        }       
