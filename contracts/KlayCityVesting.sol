...
contract Date {
    ...
    function addMonth(uint256 timestamp, uint8 month) public pure returns (uint256) {
        Time memory tm = parseTimestamp(timestamp);

        tm.month += month;
        tm.year += (tm.month - 1) / 12;
        tm.month = ((tm.month - 1) % 12) + 1;
        uint8 daysInMonth = getDaysInMonth(tm.month, tm.year);
        if (tm.day > daysInMonth) {
            tm.day = daysInMonth;
        }

        return toTimestamp(tm.year, tm.month, tm.day, tm.hour, tm.minute, tm.second);
    }
...
contract KlayCityVesting is Ownable, Pausable, Date {
    struct Info {
        address receiver;
        uint256 amount;
        uint256 claimed;
    ...
    mapping(string => Info) private infos;
    IKIP7 private orb;

    event Claim(address indexed receiver, string key, uint256 currentTimestamp, uint256 nextTimestamp, uint256 amount);
    ...
    constructor(
        ...
        Info memory marketing;

        marketing.begin = toTimestamp(2022, 3, 21);
        marketing.amount = 395733332 * 1e16;
        marketing.receiver = _marketing;
        marketing.current = marketing.begin;
        marketing.end = addMonth(marketing.begin, 24);
        marketing.claimed = 0;
        infos["Marketing"] = marketing;

        Info memory eco;

        eco.begin = toTimestamp(2023, 3, 21);

        eco.amount = 29166666666 * 1e14;
        eco.receiver = _ecoSystem;
        eco.current = eco.begin;
        eco.end = addMonth(eco.begin, 24);
        eco.claimed = 0;
        infos["EcoSystem"] = eco;

        Info memory team;

        team.begin = toTimestamp(2023, 3, 21);
        team.amount = 3750000 * 1e18;
        team.receiver = _team;
        team.current = team.begin;
        team.end = addMonth(team.begin, 12 * 4);
        team.claimed = 0;
        infos["Team"] = team;
    }

    function claim(string memory key) public whenNotPaused {
        Info storage info = infos[key];
        uint256 current = info.current;

        require(current > 0, "cant find key");
        require(current < infos[key].end, "claim ended");
        require(block.timestamp > current, "not yet");

        uint256 next = addMonth(current, 1);
        info.current = next;
        info.claimed += info.amount;

        orb.transfer(info.receiver, infos[key].amount);

        emit Claim(info.receiver, key, current, next, infos[key].amount);
    }

    function get(string memory key) public view returns (Info memory) {
        return infos[key];
    }

    function changeAddress(string memory key, address to) public onlyOwner {
        require(to != address(0), "can't change to zero address");
        Info storage info = infos[key];
        uint256 current = info.current;

        require(current > 0, "only registered address");
        info.receiver = to;
    }

    function withdraw() public onlyOwner {
        orb.transfer(owner(), orb.balanceOf(address(this)));
    }

    function withdrawTo(address wallet, uint256 amount) public onlyOwner {
        orb.transfer(wallet, amount);
    }
}
